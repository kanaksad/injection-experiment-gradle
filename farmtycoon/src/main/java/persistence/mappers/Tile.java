...
public class Tile implements persistence.Mapper {
	...
	public domain.Tile load(DBmap map) {
		TileState state;
		try {
			Class<? extends TileState> stateClass = (Class<? extends TileState>) Class
					.forName("domain.tiles." + map.getStr("state"));
			if (map.getInt("stateid") == -1) {
				state = stateClass.newInstance();
			} else {
				state = (TileState) Savable.load(
	...
	public DBmap save(Savable obj) throws SQLException {
		...
		if (tile.getState() instanceof Savable) {
			Savable state = (Savable) tile.getState();
